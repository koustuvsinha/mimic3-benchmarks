...
## clean lymphocytes
def clean_lymph(df):
    v = df.VALUE.apply(pd.to_numeric, errors="coerce").copy()
    idx = df.VALUEUOM.fillna("").apply(
        lambda s: "%" in s.lower()
    ) | df.MIMIC_LABEL.apply(lambda s: "%" in s.lower())
    v.loc[idx] = v[idx]
    return v
...
clean_fns = {
    ...
    "Weight": clean_weight,
    "Height": clean_height,
    "Lymphocytes": clean_lymph
}

generic_vals = [
    "Haemoglobin", "Bilirubin", "Magnesium", "Monocytes", "Platelets", "Red blood cell count", "White blood cell count", 
    "Creatinine", "Lactate dehydrogenase", "Mean corpuscular hemoglobin", "Lactate", "Mean corpuscular hemoglobin concentration", "Mean corpuscular volume",
    "Neutrophils", "Partial pressure of carbon dioxide", "Partial pressure of oxygen", "Pupillary response left","Pupillary response right",
    "Pupillary size left", "Pupillary size right"
]

for gv in generic_vals:
    clean_fns[gv] = clean_generic
